...
#include <mpi.h>
#include "kmc_stripping_parallel_allrates.hpp"
...
int main() {
    ...
    std::string line;
    int read_idx = 0;
    std::vector< std::string > infile_name = {"singlehexvoid_3500_interface.txt"};     
    std::vector<double> vertex_rates = {7.65954e11};
    std::vector<double> edge_rates = {1.23537e6};
    std::vector<std::string> folders = {"parallel_test_folder"};
    ...
    for (int h=0; h<(int)folders.size(); h++) {
        for (int i=0; i<iterations; i++) {
            ...
            // Redirect this rank's std::cout to its own file so per-rank output no longer
            // interleaves with every other rank's in a single shared stdout stream.
            rank_out_file.open("stdout_" + std::to_string(rank) + ".txt");
            std::cout.rdbuf(rank_out_file.rdbuf());
            ...
            if (rank == 0) {
                std::cout << "rank: " << rank << "chunk_bounds\n";
            ...
            Lattice* lattice = populate_lattice(infile_name[h], catalog_file[h], region_infile, dims, chunk_bounds, rank, procs);
            std::cout << "after populate_lattice " << std::endl;
            start = std::chrono::system_clock::now();
            std::cout << "time got \n";
            MPI_Barrier(MPI_COMM_WORLD);
            lattice_return_struct return_tuple = lattice->new_kmc_iterator(time, start, folders[h], i, last_move_tick, last_time);
            delete lattice;
