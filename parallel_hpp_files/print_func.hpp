...
/*
void reconstruct_ghost_sites(const Matrix<int>& vacancies, Lattice& lattice) {
    int lattice_pos; int x; int y; int z;
    int x_idx; int y_idx; int z_idx;

    std::vector<int> dims = lattice.dims;
    int rank = lattice.rank;
    std::vector<std::vector<int>> chunk_bounds = lattice.chunk_bounds;
    int xprocs = lattice.nprocs;
    int yprocs = lattice.nprocs;
    int nprocs = lattice.nprocs;
