...
for vnoicell in voronoi:
    ...
    for face in vnoicell['faces']:
        ...
        elif (len(all_points.shape) == 1): all_points = point_check[res]
        else: 
...
# eliminating replica points
#res = list(set(map(lambda i: tuple(sorted(i)), all_points)))
...
for polyg in polygons:
    ...
    print(f"polyg: {polyg}")
    for shape in polyg:
        #if (np.array(coord) >= [[dims[0][1]], [dims[1][1]], [dims[2][1]]]).any(): continue
        
        if (z%2 == 0):
            output = f"v {shape[0]} {shape[1]} {shape[2]} \n"
            file.write(output)
        else:
            output = f"bc {shape[0]} {shape[1]} {shape[2]} \n"
            file.write(output)
